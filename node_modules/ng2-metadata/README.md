...
[ng2-metadata](https://github.com/fulls1z3/ng2-metadata) project will continue at [@nglibs\metadata](https://github.com/nglibs/metadata).

This project has been named `ng2-metadata` as an obvious reference to `Angular2`. However `Angular 2` is now called **Angular** (with Angular 1 being AngularJS), so the name `ng2` is gonna be obsolete soon (see [Angular Presskit](https://angular.io/presskit.html)).

You can visit the http://github.com/nglibs/metadata repository for further documentation, issues and development.

**NOTE**: As a result of deprecation process, all previous versions `ng2-metadata` has been unpublished from npm registry. In order to maintain backwards compability, `ng2-metadata` will keep existing on npm registry with its final version `1.4.2-final`.
